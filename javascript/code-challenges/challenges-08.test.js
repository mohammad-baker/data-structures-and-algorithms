...
const sortByChildren = (children) => {
  // Solution code here...
  let array= children.sort((A,B) =>{
    if (A.children.length !== B.children.length) {
      return A.children.length-B.children.length;
    }
    else {
      return B.children.name-A.children.name;
    }
  });
  return array;
};
...
const citiesAtoJ = (arr) => {
  let num=/^[A-J]/g;
  // Solution code here...
  return arr.filter((vlue)=>vlue.match(num));
};
