...
const typeNum = (arr) => {
  // Solution code here...
  // that is the source of code 
  //https://stackoverflow.com/questions/44628965/filtering-numbers-out-from-an-array
 let numbFilt= arr.filter( number => parseInt(number) == number);
 return numbFilt;
};
...
const containsAnd = (arr) => {
  ...
  let appendedWords = arr.filter( person => {
      
      return (person.includes('and'));
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  let appendedWords = arr.filter( person => {
    ...
    return (!forbiddenValues.includes(person));
