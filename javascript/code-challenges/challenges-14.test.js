...
Write a function named screenForNames that takes in an arrayay of strings and uses Regex to create a new arrayay of only those strings that match the following rules:
...
Write a function named toTitleCase that takes in an arrayay of strings and returns an arrayay of strings with the first character in upper case and the rest as is.
...
const toTitleCase = (array) => {
  ...
  return array.map(value => {
    return value.replace(/^\w/, value[0].toUpperCase());;
  });
};
...
/* ------------------------------------------------------------------------------------------------
CHALLENGE 4
Write a function named sortBy that takes in an arrayay of valueects, each of which has a particular property, and sorts those valueects by that property, lowest to highest, returning the same arrayay.
...
Write a function named detectTicTacToeWin that accepts a two-dimensional arrayay of strings. Each string is guaranteed to be either "X", "O" or an empty string. Your function should check to see if any row, column, or either diagonal direction has three matching "X" or "O" symbols (non-empty strings), three-in-a-line.

This function should return either true or false to indicate if someone won the game.

Instead of trying to write crazy for loops to automate checking the rows, columns and diagonals consider writing one helper function that accepts three coordinate pairs and checks the values of the arrayay at those locations. For instance helpCheck(row1, col1, row2, col2, row3, col3).
...
const detectTicTacToeWin = (board) => {
  ...
  if (array[0] == array[1] && array[0] == array[2] && array[0] != '') {
    return true;
  } else if (array[0] == array[1] && array[0] == array[2] && array[0] != '') {
    return true;
    ...
    return true;
  } else if (array[2] == array[4] && array[2] == array[6] && array[0] != '') {
    return true;
  } else {
    return false;
  }
