...
const longestString = (arr) => {
  return arr.map(array => array.length).indexOf(Math.max(...arr.map(array => array.length)))
};
...
const firstLetters = (arr) => {
  // Solution code here...
  return arr.map(array => array.charAt(0))
};
...
const findHappiness = (arr) => {
  // Solution code here...
  return arr.filter(array => array.includes(":)"));
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let array=[]
arr.forEach(value=>{ array.push(value.replace(/[\s\-()]/g, '')) })
  return array
};
...
const allHappy = (arr) => {
  return ((arr.filter(sump => !(sump.includes(':)'))).length > 0) ? false : true);
};
